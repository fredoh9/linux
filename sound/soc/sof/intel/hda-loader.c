...
static int hda_dsp_boot_imr(struct snd_sof_dev *sdev)
{
	...
	if (chip_info && chip_info->dsp_init)
		ret = chip_info->dsp_init(sdev, 0, true);
	else
		ret = -EINVAL;
...
int hda_dsp_cl_boot_firmware(struct snd_sof_dev *sdev)
{
	...
	for (i = 0; i < HDA_FW_BOOT_ATTEMPTS; i++) {
		...
		if (chip_info->dsp_init)
			ret = chip_info->dsp_init(sdev, hext_stream->hstream.stream_tag, false);
		else
			ret = -EINVAL;
