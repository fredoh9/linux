...
static int ext_man_get_cavs_config_data(struct snd_sof_dev *sdev,
				   const struct sof_ext_man_elem_header *hdr)
{
	...
	for (i = 0; i < SOF_EXT_MAN_CAVS_CONFIG_LAST_ELEM; i++)
		switch (config_data->elems[i].token) {
		case SOF_EXT_MAN_CAVS_CONFIG_CAVS_LPRO:
			sdev->clk_config = config_data->elems[i].value;
			dev_dbg(sdev->dev, "FW clock config: %s\n", sdev->clk_config ? "LPRO" : "HPRO");
			break;
		default:
				    ...
				    const struct firmware *fw)
{
	...
	while (remaining > sizeof(*elem_hdr)) {
		...
		case SOF_EXT_MAN_ELEM_PLATFORM_CONFIG_DATA:
			ret = ext_man_get_cavs_config_data(sdev, elem_hdr);
			break;
		default:
