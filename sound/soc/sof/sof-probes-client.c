				 ...
				 size_t count, loff_t *ppos)
{
	...
	ret = sof_ipc_probe_points_info(cdev, &desc, &num_desc);
	if (ret < 0)
		goto exit;
				  ...
				  size_t count, loff_t *ppos)
{
	...
	ret = sof_ipc_probe_points_add(cdev, desc, bytes / sizeof(*desc));
	if (!ret)
		ret = count;
			  ...
			  size_t count, loff_t *ppos)
{
	...
	ret = sof_ipc_probe_points_remove(cdev, tkns, num_tkns);
	if (!ret)
		ret = count;
...
static int sof_probes_client_probe(struct virtbus_device *vdev)
{
	...
	sof_probes_card.num_links = SOF_PROBES_NUM_DAI_LINKS;
	sof_probes_card.dai_link = links;
	sof_probes_card.dev = &vdev->dev;
	...
	ret = snd_soc_fixup_dai_links_platform_name(&sof_probes_card,
						    dev_name(&vdev->dev));
	if (ret)
		return ret;

	/* register probes audio card */
	ret = devm_snd_soc_register_card(&vdev->dev, &sof_probes_card);
	if (ret < 0) {
		dev_err(&vdev->dev,
			"error: failed to register SOF probes card %d\n",
			ret);
		return ret;
	}
	...
	/* create probes debugfs dir under parent SOF dir */
