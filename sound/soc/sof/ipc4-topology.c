...
static int sof_ipc4_widget_setup_comp_pipeline(struct snd_sof_widget *swidget)
{
	...
	pipeline->msg.primary |= SOF_IPC4_GLB_MSG_TYPE(SOF_IPC4_GLB_CREATE_PIPELINE);
	pipeline->msg.primary |= SOF_IPC4_GLB_MSG_DIR(SOF_IPC4_MSG_REQUEST);
	pipeline->msg.primary |= SOF_IPC4_GLB_MSG_TARGET(SOF_IPC4_FW_GEN_MSG);
				   ...
				   size_t object_offset)
{
	...
	dev_dbg(sdev->dev, "%s rate %d channels %d sample_valid_bytes %d\n",
		__func__, params_rate(params), params_channels(params), sample_valid_bits);
	...
	if (i == available_fmt->audio_fmt_num) {
		dev_err(sdev->dev, "Unsupported rate %d channels %d sample_valid_bits %d\n",
			params_rate(params), params_channels(params), sample_valid_bits);
		return -EINVAL;
	}
...
static int sof_ipc4_route_setup(struct snd_sof_dev *sdev, struct snd_sof_route *sroute)
{
	...
	struct sof_ipc4_msg msg = {{ 0 }};
	int src_queue = 0; /* FIXME */
	int dst_queue = 0; /* FIXME */
	...
	header |= SOF_IPC4_GLB_MSG_TYPE(SOF_IPC4_MOD_BIND);
	header |= SOF_IPC4_GLB_MSG_DIR(SOF_IPC4_MSG_REQUEST);
	header |= SOF_IPC4_GLB_MSG_TARGET(SOF_IPC4_MODULE_MSG);
...
static int sof_ipc4_route_free(struct snd_sof_dev *sdev, struct snd_sof_route *sroute)
{
	...
	header = src_fw_module->man4_module_entry.id;
	header |= SOF_IPC4_MOD_INSTANCE(src_widget->instance_id);
	header |= SOF_IPC4_GLB_MSG_TYPE(SOF_IPC4_MOD_UNBIND);
	header |= SOF_IPC4_GLB_MSG_DIR(SOF_IPC4_MSG_REQUEST);
	header |= SOF_IPC4_GLB_MSG_TARGET(SOF_IPC4_MODULE_MSG);
...
static int sof_ipc4_dai_get_clk(struct snd_sof_dev *sdev, struct snd_sof_dai *dai, int clk_type)
{
	...
	for (i = 0; i < slink->num_hw_configs; i++) {
		...
		if (dai->current_config == hw_config->id) {
			hw_cfg_found = true;
			break;
		}
	}
	...
	case SOF_DAI_INTEL_SSP:
		...
		dev_err(sdev->dev, "failed to get SSP clk %d rate\n", clk_type);
		break;
	default:
