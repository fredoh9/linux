				    ...
				    unsigned long ipc_count)
{
	...
	struct device *dev = &cdev->adev.dev;
 ...
 * entries.
 */
static int sof_ipc_test_probe(struct ancillary_device *adev)
{
	...
	ipc_client_data = devm_kzalloc(&adev->dev, sizeof(*ipc_client_data),
				       GFP_KERNEL);
	if (!ipc_client_data)
		return -ENOMEM;

	ipc_client_data->buf = devm_kzalloc(&adev->dev,
	...
	ipc_client_data->dfs_root =
		debugfs_create_dir(dev_name(&adev->dev),
...
static int sof_ipc_test_cleanup(struct ancillary_device *adev)
{
	struct sof_client_dev *cdev = ancillary_dev_to_sof_client_dev(adev);
	struct sof_ipc_client_data *ipc_client_data = cdev->data;
...
static int sof_ipc_test_remove(struct ancillary_device *adev)
{
	return sof_ipc_test_cleanup(adev);
}

static void sof_ipc_test_shutdown(struct ancillary_device *adev)
{
	sof_ipc_test_cleanup(adev);
}

static const struct ancillary_dev_id sof_ipc_ancilbus_id_table[] = {
	{.name = "sof-ipc-test"},
	{ },
};
...
/*
 * No need for driver pm_ops as the generic pm callbacks in the ancillary bus
