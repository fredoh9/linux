...
/*
TODO: define complete private data
*/
...
static int sof_debug_ipc_flood_test(struct sof_ipc_client_data *ipc_client_data,
				    bool flood_duration_test,
				    unsigned long ipc_duration_ms,
				    unsigned long ipc_count)
{
	struct sof_client_dev *cdev = ipc_client_data->cdev;
	...
	while (1) {
		start = ktime_get();
		ret = sof_client_ipc_tx_message(ipc_client_data->cdev, hdr.cmd, &hdr, hdr.size,
						&reply, sizeof(reply));
		end = ktime_get();
				      ...
				      size_t count, loff_t *ppos)
{
	...
	struct sof_ipc_client_data *ipc_client_data = file->private_data;
	struct device *dev = &ipc_client_data->cdev->vdev.dev;
	...
	/* flood test */
	ret = sof_debug_ipc_flood_test(ipc_client_data, flood_duration_test,
...
MODULE_ALIAS("sof-client:sof-ipc-test-client-drv");
