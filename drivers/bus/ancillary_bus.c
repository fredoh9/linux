...
/**
 * ancillary_device_release - Release an ancillary device
 * @dev: device to release
 */
static void ancillary_device_release(struct device *dev)
{
	struct ancillary_device *adev = to_ancillary_dev(dev);
	u32 ida = adev->id;
 ...
 */
int ancillary_device_register(struct ancillary_device *adev)
{
	int ret;
	...
	adev->dev.release = ancillary_device_release;

	/* All device IDs are automatically allocated */
	ret = ida_alloc(&ancillary_dev_ida, GFP_KERNEL);
	if (ret < 0) {
...
EXPORT_SYMBOL_GPL(ancillary_device_register);
