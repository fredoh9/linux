...
/*
 * Fred: workaround to override ADL-P SDW address
 */
static const struct adr_remap intel_adlp_rvp[] = {
	{
		0x000010025d070100,
		0x000020025D071100
	},
...
static const struct dmi_system_id adr_remap_quirk_table[] = {
...
#endif
	{
		...
		.driver_data = (void *)intel_adlp_rvp,
	},
	{}
};
...
u64 sdw_dmi_override_adr(struct sdw_bus *bus, u64 addr)
{
	...
	if (dmi_id) {
		...
		for (map = dmi_id->driver_data; map->adr; map++) {
			//dev_dbg(bus->dev, "Fred: map->adr= 0x%llx addr= 0x%llx\n", map->adr, addr);
			if (map->adr == addr) {
